...
public class CheckStateImp implements CheckState<String> {
    ...
    public void taskStart(String uid, List<Paper> checkPapers, String context) {
        System.out.println("task start:" + uid + "，上下文信息：" + context);
    }
    ...
    public void taskFinish(String uid, List<Paper> checkPapers, List<Reporter> reporters, List<Paper> failedPapers, String context) {
        System.out.println("task finish:" + uid + "，上下文信息：" + context);
    }
    ...
    public void paperStart(String uid, Paper paper, String context) {
        System.out.println("paper start:" + uid + "，上下文信息：" + context);
    }
    ...
    public void paperSuccess(String uid, Reporter reporter, String context) {
        System.out.println("paper success:" + uid + "，上下文信息：" + context);
        ...
        try {
            //保存两种类型的查重报告
            //reporter.saveAsFile("C:\\codehouse\\report\\report1.html", ReportType.TEXT_WITH_CITATION); //保存全文标明引文查重报告
            //reporter.saveAsFile("C:\\codehouse\\report\\report2.html", ReportType.TEXT_WITH_ORIGINAL); //保存原文对照查重报告
            //reporter.saveAsFile("C:\\codehouse\\report\\report3.html", ReportType.SAMPLE); //保存查重报告（简洁）
            reporter.saveAsFile(savepath, ReportType.SAMPLE); //保存查重报告（简洁）
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void paperFailed(String uid, Paper paper, int code, Throwable t, String context) {
        System.out.println("paper failed:" + uid + "，上下文信息：" + context);
        t.printStackTrace();
    }
}
